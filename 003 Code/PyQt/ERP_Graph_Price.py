...
form_Main = uic.loadUiType("C:\PyQt\ERP_Graph_Price.ui")[0]

#화면을 띄우는데 사용되는 Class 선언
class Graph_PriceClass(QMainWindow, form_Main) :
    ...
    def show_graph(self):
        ...
        cur.execute("SELECT Shipment_price FROM Shipment ORDER BY Shipment_price Limit 10")
        Shipment_price = cur.fetchall()
    
        x_values = shipment_list
        y_values_price = Shipment_price
        #x_month = ['Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov']
        plt.rc('font', size = 7)
        self.graph_viewer.canvas.axes.plot(x_values, y_values_price, marker='o', color = 'blue', lw = 1) # x, y, 색깔, 선너비
        self.graph_viewer.canvas.axes.legend(['blue = price'])
        self.graph_viewer.canvas.axes.set_xticks(shipment_list)
...
if __name__ == "__main__" :
    ...
    myGraph_Price = Graph_PriceClass()
